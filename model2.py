...
# Sorting facilities
capS1 = LpVariable("capS1", 0)
capS2 = LpVariable("capS2", 0)
ss = { 'S1' : sorting(capS1, 30, 'pgb'), # vs 20/40
       'S2' : sorting(capS2, 25, 'pb') } # 30000

# Facilities
capI1 = LpVariable("capI1", 0)
capI2 = LpVariable("capI2", 0)
capC = LpVariable("capC", 0)
capP = LpVariable("capP", 0)
capG = LpVariable("capG", 0)
fs = { 'I1' : incinerator(capI1),
       'I2' : incinerator(capI2),
       'C' : compostation(capC),
       'P' : plasticrecycling(capP), # 15 vs 20
       'G' : glassrecycling(capG) } # 20 vs 40

# Landfills
ls = { 'L1' : landfill( 40000, 50),
       'L2' : landfill(500000, 70) }

cs = [ capS1, capS2, capI1, capI2, capC, capP, capG ]
